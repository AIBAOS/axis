...
本文档描述 Axis NAS 系统中更新 WebDAV 共享 API 的实现细节。

## API 端点

- **路径**: `PUT /api/v1/shares/webdav/{id}`
- **版本**: v1
- **Phase**: 218
...
{
  "success": true,
  "message": "WebDAV share updated successfully",
  "data": {
    "id": 1,
    "name": "Documents",
...
{
  "success": false,
  "error": "WebDAV share 999 not found",
  "code": "NOT_FOUND"
}
```
...
{
  "success": false,
  "error": "Only admin users can update WebDAV shares",
  "code": "FORBIDDEN"
}
```
...
### UpdateWebdavShareResponse

| 字段 | 类型 | 描述 |
|------|------|------|
| `success` | boolean | 操作是否成功 |
| `message` | string | 响应消息 |
| `data` | WebdavShareInfo | 更新后的共享信息 |
...
| `NOT_FOUND` | 404 | 共享不存在或非 WebDAV 协议 |
...
```bash
curl -X PUT "http://localhost:8080/api/v1/shares/webdav/1" \
  -H "Authorization: Bearer ADMIN_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Documents",
    "description": "更新后的文档共享文件夹",
    "public": true
  }'
```
...
### 协议验证
- 仅允许更新 `protocol = 'webdav'` 的共享
- 非 WebDAV 协议返回 404 Not Found
...
# 测试更新不存在的共享
curl -X PUT "http://localhost:8080/api/v1/shares/webdav/999" \
  -H "Authorization: Bearer ADMIN_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "NewName"}'
...
- **Phase 218** (2026-03-28): 初始实现，SQLite 持久化
