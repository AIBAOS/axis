...
|------|------|
| 最新 commit | 8cd9b4e |
| 提交时间 | 2026-03-28 18:35 UTC |
| 当前阶段 | Phase 305 备份管理界面 |
| 状态 | ✅ 已完成 |
| 阻塞项 | 无 |
...
- [x] **Phase 305 WebUI 备份管理界面** - 2026-03-28 18:35
  - BackupsView.vue: 备份管理主页 (标签页：备份任务/执行历史)
  - 备份任务卡片列表 (名称/描述/源路径/目标路径/调度/状态)
  - 状态标签 (活跃/非活跃/运行中/等待中)
  - 统计卡片 (总任务数/活跃任务/最近执行/总备份大小)
  - 搜索功能 (防抖)、状态筛选、分页支持
  - 新建/编辑备份任务模态框 (名称/描述/源路径/目标路径/备份类型/调度)
  - 立即执行备份、查看执行历史、删除备份任务
  - 执行历史表格 (任务名称/状态/开始时间/结束时间/耗时/数据量)
  - 执行状态标签 (成功/失败/运行中)
  - 路由：/backups
  - 对接 API: GET/POST/PUT/DELETE/POST /api/v1/backups/*
  - Commit: 8cd9b4e
