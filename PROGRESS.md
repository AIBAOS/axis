...
| ID | 描述 | 优先级 | 预期收益 |
|----|------|:------:|----------|
| PERF-1 | Mutex 竞争瓶颈 | 🟠 中 | 并发吞吐量提升 2-3x |
| PERF-2 | 文件上传流式处理 | 🔴 高 | 内存使用降低 80%+ |
| PERF-3 | Vec 容量预分配 | 🟡 低 | 减少 30% 内存分配 |
| PERF-4 | 数据库索引优化 | 🟠 中 | 查询速度提升 10-100x |
| PERF-5 | HTTP 响应压缩 | 🟡 低 | 网络传输减少 60-80% |
