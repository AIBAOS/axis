...
| **#19** | `users_create.rs` | 邮箱验证太弱 | ⏳ 待修复 |
| **#20** | `users_create.rs` | 用户名唯一性 Mock | ⏳ 待修复 |
| **#21** | `users_create.rs` | 密码强度验证弱 | ⏳ 待修复 |
| **#22** | `auth.rs/auth_login.rs` | `unwrap()` 可能 panic | ⏳ 待修复 |
| **#23** | `printers_create.rs` | 无 IP 格式验证 | ⏳ 待修复 |

### 🟡 低严重度（技术债务）

| Bug | 文件 | 问题描述 |
|-----|------|---------|
| **#24** | 多 handlers | 30+ 处 `unwrap()` 在 Mock 数据上 |
