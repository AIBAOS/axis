...
/// FTP 共享详情响应
#[derive(Serialize)]
pub struct FtpShareDetailResponse {
    pub success: bool,
    pub data: FtpShareDetail,
}

/// FTP 共享详情
#[derive(Serialize)]
pub struct FtpShareDetail {
    pub id: u64,
    pub name: String,
    pub path: String,
...
pub async fn get_ftp_share(
    ...
    if !is_admin {
        return Ok(HttpResponse::Forbidden().json(ErrorResponse {
            success: false,
            error: "Only admin users can view FTP share details".to_string(),
    ...
    match repo.get_share_by_id(share_id) {
        Ok(Some(share)) => {
            ...
            Ok(HttpResponse::Ok().json(FtpShareDetailResponse {
                success: true,
                data: detail,
            }))
        }
