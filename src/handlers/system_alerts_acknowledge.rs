...
pub async fn acknowledge_system_alert(
    ...
    if !is_admin {
        return Ok(HttpResponse::Forbidden().json(ErrorResponse {
            success: false,
            error: "Only admin users can acknowledge system alerts".to_string(),
    ...
    // 8. 验证告警状态（仅 active 状态可确认）
    if alert.status != "active" {
        return Ok(HttpResponse::BadRequest().json(ErrorResponse {
            success: false,
            error: format!("Alert {} is not in active status", alert_id),
    ...
    alert.acknowledged_at = Some(now.clone());
    alert.acknowledged_by = Some(current_user);

    // 10. 返回确认成功
    Ok(HttpResponse::Ok().json(AcknowledgeAlertResponse {
        success: true,
        message: "System alert acknowledged successfully".to_string(),
        data: alert.clone(),
    }))
}
