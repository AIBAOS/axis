// Phase 200: 系统通知标记已读 API (POST 版本)
// POST /api/v1/system/notifications/{id}/mark-read — 标记系统通知为已读
...
/// 标记系统通知为已读（Phase 200 - POST 版本）
/// - JWT 认证，登录用户可访问
/// - 验证通知 ID 存在性（404 Not Found）
/// - 验证通知归属：只能标记自己的通知或系统全局通知
/// - 如果通知已读，返回 409 Conflict
/// - 成功返回 200 OK + 通知完整信息
pub async fn mark_notification_read(
    ...
    match repo.get_notification_by_id(notification_id) {
        Ok(Some(notification)) => {
            // 4. 验证通知归属：只能标记自己的通知或系统通知
            let is_system_notification = notification.target_user_id.is_none();
            let is_own_notification = notification.target_user_id == _claims.user_id;
            
            if !is_system_notification && !is_own_notification {
                return Ok(HttpResponse::Forbidden().json(ErrorResponse {
                    success: false,
                    error: "无权标记此通知为已读".to_string(),
            ...
            match repo.mark_as_read(notification_id) {
                Ok(true) => {
                    ...
                    match repo.get_notification_by_id(notification_id) {
                        ...
                        _ => Ok(HttpResponse::Ok().json(MarkReadResponse {
                            success: true,
