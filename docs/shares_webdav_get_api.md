...
本文档描述 Axis NAS 系统中获取 WebDAV 共享详情 API 的实现细节。

## API 端点

- **路径**: `GET /api/v1/shares/webdav/{id}`
- **版本**: v1
- **Phase**: 216
...
{
  "success": false,
  "error": "Only admin users can view WebDAV share details",
  "code": "FORBIDDEN"
}
```
...
### WebdavShareDetailResponse

| 字段 | 类型 | 描述 |
|------|------|------|
| `success` | boolean | 操作是否成功 |
| `data` | WebdavShareDetail | 共享详情信息 |
...
```bash
curl -X GET "http://localhost:8080/api/v1/shares/webdav/1" \
...
# 测试获取不存在的共享
curl -X GET "http://localhost:8080/api/v1/shares/webdav/999" \
  -H "Authorization: Bearer ADMIN_JWT_TOKEN"
...
- **Phase 216** (2026-03-28): 初始实现，SQLite 持久化
