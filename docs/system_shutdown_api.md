...
本文档描述 Axis NAS 系统中系统关机 API 的实现细节。

## API 端点

- **路径**: `POST /api/v1/system/shutdown`
- **版本**: v1
- **Phase**: 230
...
{
  "success": false,
  "error": "Only admin users can shutdown the system",
  "code": "FORBIDDEN"
}
```
...
| `message` | string | 响应消息 |
| `shutdown_at` | number | 关机时间戳（Unix 时间戳） |
...
```json
{
  "success": true,
  "status": "scheduled",
  "message": "System shutdown scheduled in 0 seconds",
  "shutdown_at": 1711600000
}
```
...
- 计算关机时间戳：`now + delay_seconds`
- 返回 Unix 时间戳（秒）

### 系统关机实现
- 当前为模拟实现，仅返回计划信息
- 实际实现可调用系统命令：
  - Linux: `shutdown -h +<minutes>` 或 `systemctl poweroff`
  - 需要适当的系统权限
...
- `POST /api/v1/system/restart` - 系统重启
...
- **Phase 230** (2026-03-28): 初始实现，模拟关机计划
